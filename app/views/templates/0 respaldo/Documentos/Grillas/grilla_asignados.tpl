<table id="tablaPrincipal" class="table table-hover table-striped table-bordered  table-middle dataTable no-footer">
    ...
    {foreach $arrResultado as $itm}
        <tr>
            ...
            <td align="center">
                {if (Fechas::diffDias(date('Y-m-d'),$itm->fc_fecha_ingreso_partes_solicitud,true) - $itm->total_dias_feriados) >= 7}
                    <div class="label label-danger" style="font-size:12px">{Fechas::diffDias(date('Y-m-d'),$itm->fc_fecha_ingreso_partes_solicitud,true) - $itm->total_dias_feriados}</div>
                {else}
                    {Fechas::diffDias(date('Y-m-d'),$itm->fc_fecha_ingreso_partes_solicitud,true) - $itm->total_dias_feriados}
                {/if}</td>
            ...
            <td nowrap class="text-center">
                ...
                <button data-toggle="tooltip" type="button" class="btn btn-sm btn-success btn-flat" title="Ver Detalle" onClick="xModal.open('{$smarty.const.BASE_URI}/Documento/revisarDocumento/{$itm->id_solicitud}','Revisar Documento',85);">
